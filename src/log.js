...
class log {
  ...
  currdate(set = false) {
    var d = new Date(),
      dformat =
        [this.pdleft(d.getDate() + 1), this.pdleft(d.getMonth()), d.getFullYear()].reverse().join('/') +
        ' ' +
        [this.pdleft(d.getHours()), this.pdleft(d.getMinutes()), this.pdleft(d.getSeconds())].join(':');
    console.log(dformat);
    if (!set) {
      return dformat;
